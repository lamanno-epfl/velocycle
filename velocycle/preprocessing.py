...
def preprocess_for_phase_estimation(
    ...
    metapars = dict(
        ...
        Db=design_mtx.T[:, None, :].to(device),
...
def preprocess_for_velocity_estimation(
    ...
    noisemodel="Poisson",
    condition_on={},
    μγ=torch.tensor(0.3).float(),
    σγ=torch.tensor(0.1).float(),
    μβ=torch.tensor(0.65).float(),
    σβ=torch.tensor(0.1).float(),
    μΔν=torch.tensor(0).float(),
    σΔν=torch.tensor(0.01).float(),
    gamma_alpha=torch.tensor(1.0).float(),
    gamma_beta=torch.tensor(2.0).float(),
    model_type: str = "normal",
    rho_mean = torch.tensor(5.0),
